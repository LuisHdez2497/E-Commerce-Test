                ...
                <form @submit="sendForm">
                    <div class="overflow-hidden shadow-2xl border-2 sm:rounded-md">
                        ...
                        <div class="bg-gray-50 px-4 py-3 text-right sm:px-6">
                            <router-link to="/admin/productos" class="inline-flex justify-center rounded-md border border-transparent bg-red-600 py-2 px-4 text-xl font-medium text-white shadow-sm hover:bg-red-700 mr-5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" >Volver</router-link>
                            <button type="submit" class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-xl font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Guardar</button>
                        </div>
                    </div>
                </form>
            </div>
...
export default {
    ...
    mounted() {
        this.getProducts();
        this.product.id = this.filterProduct.id;
        this.product.name = this.filterProduct.name;
        this.product.description = this.filterProduct.description;
        this.product.price = this.filterProduct.price;
        this.product.status = this.filterProduct.status == 1 ? true : false;
    }
};
</script>
